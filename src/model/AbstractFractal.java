...
public abstract class AbstractFractal {
    ...
    public AbstractFractal() {
        affine = Collections.synchronizedList(new ArrayList<>());
    }
    ...
    public void initImage(int width, Color b, Color f) {
        ...
        this.width = width;
        font = new Font("Ariel", Font.PLAIN, 100);
    }

    /**
     * Define Affine transformation
     *
     * Note the angle are defined in clock wise direction
     *
     */
    ...
    public BufferedImage showMap() {
        ...
        g.drawString("Map", width - 200, 100);
    ...
    protected double[] parameters(double r, double s,
        ...
        p[4] = e * width;
        p[5] = f * width;
//       System.out.println(parameter2str(p));
        return p;
    }

    public String parameter2str(double parameter[]) {
        StringBuilder sb = new StringBuilder();
        sb.append("{");
        for (int i = 0; i < parameter.length - 1; i++) {
            sb.append(parameter[i]).append(",");
        }
        sb.append(parameter[parameter.length - 1]).append("}");
        return sb.toString();
    }
